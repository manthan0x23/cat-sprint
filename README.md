...
4. Cron (free, Cloudflare Workers): set `APP_URL` in `cron-worker/wrangler.jsonc`, then
   ```bash
   cd cron-worker && npm i
   npx wrangler login
   npx wrangler secret put CRON_SECRET   # same value as Vercel
   npm run deploy                        # fires every 30 min; `npm run tail` to watch logs
   ```
