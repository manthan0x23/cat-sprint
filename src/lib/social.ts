...
export async function sharedStats(userId: string) {
  ...
  return {
    ...
    today: s.today,
    mocksTaken: s.mocksTaken,
    mocksPlannedLeft: s.mocksPlannedLeft,
    lastMockScore: s.profile.showMocks ? s.mocks.at(-1)?.score ?? null : null,
    mocks: s.profile.showMocks ? s.mocks.map((m) => ({ date: m.date, name: m.name, score: m.score })) : null,
